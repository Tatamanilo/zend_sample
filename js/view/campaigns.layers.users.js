var LayersUsers = Backbone.View.extend({
    prefix: 'groupsEdit',
    useDialog: true,
    idCampaign: null,
    users: [],
    ...
    deleteUser: function (event) {
        ...
        SMP.makeRequest(
            ...
            function (data) {
                if (SMP.isset(data.result) && (data.result == 1)) {
                    //self.refreshTable();
                    $("#layerUserRow" + $(event.currentTarget).attr("uid")).remove();
                }
            },
            'users.openEdit'
        );
    ...
    addUsersGroup: function (event) {
        ...
        SMP.makeRequest(
            '/campaigns/layers/addusersgroup',
            {
                idLayer: self.idLayer,
    ...
    refreshTable: function () {
        ...
        SMP.makeRequest(
            ...
            function (data) {
                if (SMP.isset(data.result) && (data.result == 1)) {
                    $('#lusers' + self.idLayer).replaceWith(data.html);
                    self.delegateEvents();
