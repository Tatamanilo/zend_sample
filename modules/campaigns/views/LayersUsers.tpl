...
<div class="form-group">
    ...
    <div class="row" >
        ...
        <div class="col-lg-4">
            <button class="btnadd btn btn-default addUser" lid="{$idLayer}"><i class="fa fa-plus"></i></button>
        </div>
    </div>
</div>
...
<form id="usersToDelete{$idLayer}" class="usersToDeleteForm">
{/if}
<table cellspacing="0" cellpadding="0" border="0" id="lusers{$idLayer}" class="table table-hover">
    <thead>
        <tr>
            <th></th>
            <th>Email</th>
    ...
    <tr id="layerUserRow{$user.idUser}"  class="{if $user.status!='active'}text-muted{/if}">
        <td class="tableitem">
...
{if !$listOnly}
<input type="hidden" name="idLayer" value="{$idLayer}" />
